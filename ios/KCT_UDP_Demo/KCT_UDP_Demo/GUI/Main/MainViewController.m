...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == self.serInfoTableView) {
        ...
        NSString *title = [dic objectForKey:@"ssip"];
        NSString *ssname = [dic objectForKey:@"ssname"];
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue mainQueue] completionHandler:^(NSURLResponse * _Nullable response, NSData * _Nullable data, NSError * _Nullable connectionError) {
        ...
        if (data) {
            ...
            if ([result intValue] == 200) {
                ...
                self.ipArrays = [NSMutableArray arrayWithArray:ips];
                _ipSelectIndex = 0;
...
- (void)streamPush
{
    ...
    NSString *streamPushUrl = [NSString stringWithFormat:@"http://%@:%@/udprelay/v1/streamPush?streamId=%@&streamCan=%ld",ipAddr,port,self.sessionId,_segmentSelectIndex];

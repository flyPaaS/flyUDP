...
+ (double)getAverageRtt:(NSArray *)pingItems {
    ...
    [pingItems enumerateObjectsUsingBlock:^(STDPingItem *obj, NSUInteger idx, BOOL *stop) {
        if (obj.status != STDPingStatusFinished && obj.status != STDPingStatusError) {
            allCount ++;
            if (obj.status == STDPingStatusDidReceivePacket && obj.timeMilliseconds > 0) {
                receivedCount ++;
                result += obj.timeMilliseconds;
            }
        }
    }];
    
    result = result/receivedCount;
    return result;
}
...
@property(nonatomic, strong)void(^callbackHandler)(STDPingItem *item, NSArray *pingItems,NSString *addr,NSInteger index);
...
+ (STDPingServices *)startPingAddress:(NSString *)address
                      callbackHandler:(void(^)(STDPingItem *item, NSArray *pingItems,NSString *addr,NSInteger index))handler {
    STDPingServices *services = [[STDPingServices alloc] initWithAddress:address index:0];
    services.callbackHandler = handler;
    [services startPing];
    return services;
}

- (void)startPingWithCallBack:(void(^)(STDPingItem *item, NSArray *pingItems,NSString *addr,NSInteger index))handler {
    self.callbackHandler = handler;
    [self startPing];
}

- (instancetype)initWithAddress:(NSString *)address index:(NSInteger)index{
    self = [super init];
    if (self) {
        self.timeoutMilliseconds = 20;
        self.maximumPingTimes = 10;
...
- (void)_handlePingItem:(STDPingItem *)pingItem {
    ...
    if (_repingTimes < self.maximumPingTimes - 1) {
        if (self.callbackHandler) {
            self.callbackHandler(pingItem, [_pingItems copy],self.address,self.index);
        }
        _repingTimes ++;
...
- (void)st_simplePing:(STSimplePing *)pinger didStartWithAddress:(NSData *)address {
    ...
    if (!_hasStarted) {
        ...
        if (self.callbackHandler) {
            self.callbackHandler(pingItem, nil,self.address,self.index);
        }
        _hasStarted = YES;
    }
...
- (void)st_simplePing:(STSimplePing *)pinger didFailWithError:(NSError *)error {
    ...
    if (self.callbackHandler) {
        self.callbackHandler(errorPingItem, [_pingItems copy],self.address,self.index);
    }

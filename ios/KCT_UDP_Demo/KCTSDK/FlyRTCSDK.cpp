...
#if kMediaLog
    g_pAVEngine->set_debug_level(0x00ff);
    
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentsDirectory = [paths objectAtIndex:0];
    documentsDirectory = [documentsDirectory stringByAppendingPathComponent:@"MediaEngineLog"];
...
void Vigo_start(const char *ip, int audioport, int videoport, void *hLocal, void *hRemote,int remoteWidth,int remoteHeight)
{
    ...
    if (g_pAVEngine)
    {
        ...
        if (g_enableVideo)
        {
            ...
            stVideoRenderConfig.remoteWidth = remoteWidth;
            stVideoRenderConfig.remoteHeight = remoteHeight;
...
int Vigo_getLoudspeakerStatus()
{
    return g_pAVEngine->get_loudspeaker_status();
}

void Vigo_setCamera(int CameraIndex)
{
    ME_video_camera_param_t param;
    memset(&param, 0, sizeof(ME_video_camera_param_t));
    g_pAVEngine->video_get_camera_state(&param);
    param.usCameraIdx = CameraIndex;
    g_pAVEngine->video_set_camera(&param);
}
